...
class MultiSpeakerTacotron():
    ...
    def initialize(self, inputs, input_lengths, speaker_ids, mel_targets=None, stop_token_targets=None, linear_targets=None,
        ...
        with tf.variable_scope('inference') as scope:
            ...
            speaker_embedding_table = tf.get_variable(
                'speaker_embedding', [377, hp.embedding_dim], dtype=tf.float32,
                initializer=tf.truncated_normal_initializer(stddev=0.5))
            tiled_speaker_id = tf.tile(tf.expand_dims(speaker_ids, axis=1), [1, tf.shape(inputs)[1]])
            embedded_speakers = tf.nn.embedding_lookup(speaker_embedding_table, tiled_speaker_id)
            embedded = tf.concat([embedded_inputs, embedded_speakers], axis=-1)
            ...
            encoder_outputs = encoder_cell(embedded, input_lengths)
            ...
            log('  embedding:                {}'.format(embedded.shape))
